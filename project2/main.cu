...
int main(int argc, char* argv[])
{
    ...
    // After our integrated merge, data is always in d_in (in_d)
    if (algo == 2) {
        cuda_ret = cudaMemcpy(out_h, out_d, num_elements * sizeof(float), cudaMemcpyDeviceToHost);
        if (cuda_ret != cudaSuccess) FATAL("Unable to copy memory to host");
    } else {
        cuda_ret = cudaMemcpy(out_h, in_d, num_elements * sizeof(float), cudaMemcpyDeviceToHost);
        if (cuda_ret != cudaSuccess) FATAL("Unable to copy memory to host");
    }
